...
for index, line in enumerate(table_data):
    ...
    for i in range(future_predictions.shape[1]):
        table_save["extend_data" + str(i + 1)].loc[index] = future_predictions[:, i].flatten()
    ...
    plt.ylabel('Value')
    plt.legend()
    plt.savefig("save_pic1/" + str(select_index) +"_"+ str(index) + ".svg")
    plt.show()

    table_save.to_csv("/workspace/table_save3/table_save_"+str(index)+"_.csv")
